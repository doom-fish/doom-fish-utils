...
/// # Example
/// ```
/// use doom_fish_utils::ffi_string::ffi_string_from_buffer;
///
/// let result = unsafe {
...
/// The FFI function allocates the string (typically via Swift's `strdup`) and
/// this helper takes ownership and frees it using the caller-supplied
/// `free_fn`.
///
/// # Arguments
/// * `ffi_call` - A closure that returns an owned C string pointer (or null)
/// * `free_fn`  - A function that frees the pointer returned by `ffi_call`.
///   Each bridging crate passes its own `_free_string` `extern "C"` here
///   (e.g. `apple_cf::ffi::acf_free_string`,
///   `screencapturekit::ffi::sc_free_string`).
///
/// # Returns
...
///
/// # Safety
/// The caller must ensure the returned pointer was allocated by the Swift /
/// Objective-C side using a strategy that `free_fn` correctly releases.
/// The pointer is freed via an RAII guard, so a panic in `from_utf8_lossy`
/// (extremely rare — only OOM) does not leak the bridge-allocated buffer.
pub unsafe fn ffi_string_owned<F, Free>(ffi_call: F, free_fn: Free) -> Option<String>
where
    F: FnOnce() -> *mut i8,
    Free: FnOnce(*mut i8),
{
    /// RAII guard: releases the bridge-allocated buffer on drop, including
    /// during panic unwind. Without this, a panic between `CStr::from_ptr`
    /// and the explicit free call (e.g. allocator failure inside
    /// `from_utf8_lossy`) would leak the buffer.
